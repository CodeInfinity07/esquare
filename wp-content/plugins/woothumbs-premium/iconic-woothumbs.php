<?php
/**
 * Plugin Name: Thumbnify by Junaid Khalid
 * Description: Multiple images per variation and customisable image gallery with video support in different ways and sources.
 * Version: 1.0.0
 * Author: Junaid Khalid
 * Author URI: https://wa.me/923100509070
 * Text Domain: thummbnify
...
class Iconic_WooThumbs {
	...
	public function generate_variation_data( $product = false ) {
		...
		foreach ( $variations as $variation_id ) {
			$variation_images = $this->get_all_image_sizes( $variation_id );
		...
		<script>
			(function( $, document ) {
				window.iconic_woothumbs_variations_data = <?php echo wp_json_encode( $variation_data ); ?>;
			}( jQuery, document ));
		</script>
		<?php
	}
	...
	public static function get_image_loop_data( $image, $index ) {
		...
		return array(
			'src'                       => ! empty( $image['src'] ) && 0 === $index ? $image['src'] : 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs=',
			'data-iconic-woothumbs-src' => empty( $image['src'] ) || 0 === $index ? false : $image['src'],
			'class'                     => 'iconic-woothumbs-images__image no-lazyload skip-lazy',
